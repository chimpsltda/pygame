...
class Objective(pygame.sprite.Sprite):
    def __init__(self):
        super().__init__()
        # Carregar todas as sprites necessárias
        self.sprites = [pygame.image.load(f'texturas/objetos/objetivo{i}.png') for i in range(15)]
        self.current_sprite = 0
        self.surf = self.sprites[self.current_sprite]  # Iniciar com a sprite 'objetivo0.png'
    ...
    def update(self, player_rect):
        if self.rect.colliderect(player_rect):
            ...
            if elapsed_time >= OBJECTIVE_TIME:
                self.current_sprite = 14  # Mantém a última sprite
                self.surf = self.sprites[self.current_sprite]
                self.kill()
            else:
                progress = elapsed_time / OBJECTIVE_TIME
                sprite_index = int(progress * 14)  # Calcula o índice da sprite com base no progresso
                self.current_sprite = sprite_index
                self.surf = self.sprites[self.current_sprite]
