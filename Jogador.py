...
class Player(pygame.sprite.Sprite):
    def __init__(self):
        ...
        self.health_bar_visible_timer = 0
    ...
    def draw_health_bar(self, screen):
        if self.health_bar_visible_timer > 0:
            # Calcula a largura da barra de vida com base na saúde atual
            bar_width = 50 * (self.health / self.max_health)
            bar_height = 5  # Altura da barra de vida

            # Define a posição da barra de vida acima da sprite do jogador
            bar_x = self.rect.x + (self.rect.width / 2) - (bar_width / 2)
            bar_y = self.rect.y - 10  # 10 pixels acima da sprite do jogador
    ...
    def change_weapon(self, weapon_index):
        ...
        self.damage = self.armas[self.arma_atual].DAMAGE + self.damage_extra
